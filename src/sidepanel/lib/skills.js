...
async function callSkillHelper(messages) {
...
export async function generateSkillDraft(input) {
    const raw = await callSkillHelper([
        {
            ...
            conversationId: "",
            role: "user",
            content: `Create a reusable browser-assistant skill.\n` +
    ...
    const parsed = JSON.parse(escapeJson(raw));
    return parsed;
}
export async function refineSkillPrompt(input) {
    const raw = await callSkillHelper([
        {
            id: "skill-refine",
            conversationId: "",
            role: "user",
            content: `Rewrite the prompt for this saved skill.\n` +
    ...
    return JSON.parse(escapeJson(raw));
}
