...
export function estimateMessageTokens(m: Message): number {
  ...
  for (const a of m.attachments ?? []) n += estimateTokens(a.text);
  for (const r of m.toolResults ?? []) n += estimateTokens(r.content);
  for (const c of m.toolCalls ?? []) n += estimateTokens(JSON.stringify(c.input || {}));
  return n + 4;
}

// Dedup repeated page/selection attachments — only the latest copy keeps full text.
export function dedupAttachments(messages: Message[]): Message[] {
  const seen = new Map<string, number>();
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (!m.attachments) continue;
    for (const a of m.attachments) {
      if (a.kind !== "page" && a.kind !== "selection") continue;
  ...
  return messages.map((m, i) => {
    ...
    const next = m.attachments.map((a) => {
      if (a.kind !== "page" && a.kind !== "selection") return a;
      const key = `${a.kind}:${a.url ?? ""}`;
      if (seen.get(key) === i) return a;
...
export async function buildSendArray(
  conv: Conversation,
  allMessages: Message[],
  settings: Settings
): Promise<BuildResult> {
...
function assemble(summary: string, recent: Message[]): Message[] {
  const out: Message[] = [];
  if (summary) {
    out.push({
      id: "summary",
  ...
  for (const m of dedupAttachments(recent)) out.push(m);
  return out;
}

function sum(msgs: Message[]): number {
