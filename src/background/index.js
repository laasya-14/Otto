...
chrome.commands.onCommand.addListener(async (command, tab) => {
    ...
    const tabId = tab?.id;
    if (tabId != null) {
        chrome.sidePanel.open({ tabId }).catch((e) => console.warn("[Side] sidePanel.open failed:", e));
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    if (msg?.type === "FETCH_FOCUS_COMPOSER") {
        chrome.storage.session.get("focusComposerAt").then(async (r) => {
            const at = r.focusComposerAt;
            if (at)
                await chrome.storage.session.remove("focusComposerAt");
    ...
    if (msg?.type === "BG_GET_SELECTION" || msg?.type === "BG_GET_PAGE") {
        activeTabId().then(async (tabId) => {
            ...
            const r = await askContent(tabId, msg.type === "BG_GET_SELECTION"
                ? { type: "GET_SELECTION" }
                : { type: "GET_PAGE" });
            sendResponse(r);
        });
